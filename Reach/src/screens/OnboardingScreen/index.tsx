...
const OnboardingScreen = ({ navigation }: StackNavigationProps) => {
  ...
  return (
    ...
    >
     ...
     <View className='mt-32 h-11 bg-[#2061C7] w-[70%] mx-14 rounded-[50px]'><Text className='text-center text-[13px] text-white py-3'>Let's Get Started</Text></View>
    </TouchableOpacity>
      </View>
      <View className='flex-1 pt-5 relative'>
        <Image
      ...
      <View className='mx-9  flex justify-center items-center z-10'>
        ...
        <Image
          source={Image2}
          resizeMode="cover"
          className="mx-20 absolute top-32 z-20 w-52 h-52"
        />
      <TouchableOpacity onPress={()=> navigation.navigate('SignInScreen')} >
     <View className='absolute top-96 h-11 bg-[#2061C7] w-[70%] mx-14 rounded-[50px]'><Text className='text-center text-[13px] text-white py-3'>Start</Text></View>
    </TouchableOpacity>
      </View>
    </Swiper>
  </View>
